...
function ViewIrrigation() {
  ...
  const handleDelete = (id) => {
    fetch(`http://localhost:8080/irrigation/${id}`, {
      ...
      .then(() => {
        const updatedIrrigations = irrigations.filter((irrigation) => irrigation.id !== id);
  ...
  const handleSaveEdit = () => {
    ...
    const formattedIrrigationDate = new Date(editIrrigation.irrigationDate).toISOString().split('T')[0];
  
    const updatedIrrigation = { ...editIrrigation, irrigationDate: formattedIrrigationDate };
